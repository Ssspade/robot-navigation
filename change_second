...
def mouseCallback(event, x, y, flags, null):
    ...
    if previous_x > 0:
        ...
        #*************修改部分*************************8
        update(particles, weights, z=zs, landmarks=landmarks)
...
def update(particles, weights, z,landmarks):
    weights.fill(1.)
    for i, landmark in enumerate(landmarks):
        distance = np.power((particles[:, 0] - landmark[0]) ** 2 + (particles[:, 1] - landmark[1]) ** 2, 0.5)/8
        #****************修改部分：泊松分布计算weight*****************
        z[i]/=8
        z[i] = np.trunc(z[i])
        weights *=scipy.stats.poisson.pmf(z[i], distance)
